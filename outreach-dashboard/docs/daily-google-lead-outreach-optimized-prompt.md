...
- For Alibaba Mail first touch, attach exactly the approved FLEXTAIL 2026 catalog, monthly product roadmap, and distributor network overview. Verify the three exact filenames and completed upload state in the active composer before Send; missing, extra, or pending attachments fail closed before the click and trigger safe channel fallback. Mention the attachments once in the email body.
