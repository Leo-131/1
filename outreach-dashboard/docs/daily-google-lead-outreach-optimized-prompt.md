...
- 已登录的 LinkedIn Sales Navigator 可在副 Chrome/CDP 9224 中用于检索并验证真实买家，以及在控件受支持时执行关注、点赞、连接请求或消息。Navigator 权限不代表平台真正无限；遇到任何可见限额、风控、CAPTCHA、身份不确定、重复或确认缺失必须停用该目标。不得用姓名或域名模式猜测邮箱，邮箱仍须由第一方官网精确交叉验证。
