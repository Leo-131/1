...
- After each email batch, run a bounded scan of the authenticated Inbox for DSN/delivery failures. Immediately rewrite a matched address/company to `bounced`, remove it from confirmed totals, and permanently block automatic resend. Ordinary automated acknowledgements prove delivery only; never count them as buyer replies or opportunities.
