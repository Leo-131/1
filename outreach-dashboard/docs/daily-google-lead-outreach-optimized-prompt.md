...
- Personal-domain addresses are rejected by default. Permit a narrow exception only for an ICP-qualified `sales_agency` when its own HTTPS website explicitly publishes the exact address as the principal/agency business contact, the evidence URL has the same first-party host, and both `official_supplier_email_verified` and `official_public_business_email` statuses are present. Search results, directories, guessed addresses, non-agency targets, and mismatched evidence hosts never qualify.
- When the same live first-party page also exposes a contact form and normalization records `official_contact_form_verified`, preserve the personal-domain agency email route only if that exact verification record includes `public_business_email`. Never infer email eligibility from a form signal alone.
...
- Do not strand an already first-party-cross-verified social profile behind a channel-specific `website_contact_unreachable_skip` or historical website failure circuit. Permit that social row only when the website evidence proves no send/submit/customer interaction; never bypass `failed_open`, `send_unconfirmed`, any physical click, or the Shanghai-day company retry circuit.
...
- 邮件后的每一次社媒触达都必须在账本中保存明确的副渠道状态和证据。验证器或传输层若只返回 reason/error 而缺少 `sendStatus`，必须规范化为发送前 `failed_open`；禁止丢失为 `social_result_missing`，也禁止提升为已确认。
- 同一精确任务在当前上海日期一旦记录 `website_contact_ready` 或 `website_contact_unreachable_skip`，必须视为该任务当日终态并从后续批次移除；允许同企业真正不同且已验证的官方渠道，但禁止相同任务反复占用队首、阻塞后续客户。
- 企业官网链接到创始人或员工的 LinkedIn `/in/` 个人资料只证明关联，不等于企业官方触达授权。企业多渠道回退只能使用身份已验证的 LinkedIn `/company/` 组织页；个人买家触达必须走独立的买家身份验证流程。
- 已登录的 LinkedIn Sales Navigator 可在副 Chrome/CDP 9224 中检索并验证真实买家、读取 LinkedIn 页面明确显示的完整邮箱，以及在控件受支持时执行关注、点赞、连接请求或消息。完整邮箱也可由企业第一方官网对同一身份和地址精确交叉验证；遮罩、残缺、模式生成、推测、第三方富集或搜索摘要邮箱一律记为 `email_not_exposed`/未验证。保存个人页、证据 URL、验证方法和上海时间。Navigator 权限不代表平台真正无限；遇到可见限额、风控、CAPTCHA、身份不确定、重复或确认缺失必须停用该目标。
- 对 LinkedIn 已确认的合格买家采用最短安全链路：优先读取 LinkedIn 明确显示的完整邮箱；未显示时只查询一个企业第一方官网/供应商页面的精确公开地址；完成企业与收件人历史/同日去重后，直接在已登录阿里邮箱自动发送并核验 Sent。该链路不等待客户池补充、额外社媒动作或人工审核，但绝不绕过身份、精确邮箱、主题/正文/附件、去重、投递和确认门禁。
- 具名买家的 LinkedIn `/in/` 个人页只有在企业自己的 HTTPS 官网直接链接该精确主页、明确买家姓名且证据同域时，才可进入 Navigator 买家流程；执行时必须在 LinkedIn 页面再次匹配姓名，并将结果记为具名买家触达，不能把个人页伪装成企业官方主页。
...
- When a stable task ID was previously recorded against a failed website-only URL, do not let that ID suppress a newly first-party-verified official LinkedIn/Facebook/Instagram URL on a later Shanghai day. Compare exact target routes, retain all company/identity/duplicate/uncertainty gates, and keep the same-day terminal circuit closed.
- Treat transient Windows `UNKNOWN`, busy, access, or permission errors while updating reply reconciliation ledgers and public mirrors as bounded retryable artifact writes. Preserve completed evidence and never convert a temporary reader lock into a manual rerun or lost result.
...
- For Alibaba Mail first touch, include the approved FLEXTAIL 2026 catalog, monthly product roadmap, and distributor network overview. Default to the three exact owner-provided Google Drive URLs in the body and verify every URL before Send; use local attachments only when all three exact approved filenames can be visibly verified as fully uploaded. Missing or unverified collateral fails closed before the click.
- End every development email with the exact approved signature: `Sincerely`, `Best Regard`, `Leo Liu`, `Overseas Business Unit`, `Tel/whatsapp: +86 17321028184`, `Email: Leo@flextailgear.com`, `SHANGHAI FLEXTAIL TECHNOLOGY CO.,LTD.`, `5th Floor, No. 68, Yincheng Middle Road, Pudong New Area, Shanghai (Times Finance Center)`.
...
- Email subjects must position the outreach as FLEXTAIL's global channel-partner recruitment, never as a generic `retail partnership` or assortment announcement. Use `FLEXTAIL global brand representation | <Company>` for sales/brand agencies, `FLEXTAIL global distribution partnership | <Company>` for distributors and key accounts, and the combined distribution-and-representation wording only when the customer type is genuinely ambiguous. Preserve the selected subject exactly through compose, pre-send consistency checks, Sent verification, and ledger evidence.
...
- Alibaba Mail web UI is the only permitted email-sending surface. Never open, inspect, or control Foxmail or another desktop/alternate email client. If an alternate provider is requested or configured, fail closed before composing or sending and continue only with safe first-party-verified non-email channels.
...
- Alibaba may require a first click to focus a recipient chip and a second click to expose the full-address tooltip. If the committed chip itself renders the complete exact intended email, accept that exact text as recipient identity evidence; otherwise allow at most two scoped chip clicks and require the tooltip to show the exact intended address before Send. A chip-click timeout is a pre-send technical failure: do not click Send, preserve the populated composer, and do not classify it as `send_unconfirmed`.
- Recognize committed Alibaba recipients in both Ant Design selection items and overflow items, but only inside the active compose recipient row; a chip is proof only when its own complete rendered text exactly equals the intended email or its tooltip exposes that exact email.
...
- For a user-supplied filtered Sales Navigator people search, treat each visible row's exact buyer name, qualifying buyer/procurement/category title, and current company as sufficient buyer-source verification. Connect serially through the row's own action menu, deduplicate by profile/name, and do not impose an application-level numeric cap. Use the verified queue, a 45-minute resumable checkpoint, and visible platform limits/CAPTCHA/restrictions/missing confirmation as stopping conditions. A Connect request is engagement evidence only and never counts toward the 100 confirmed-customer target. Capture an email only when LinkedIn visibly exposes the complete address or the exact address is cross-verified on the company's first-party website; otherwise retain the verified social route without guessing.
- A represented brand's official social page does not become the agency's social identity. Use it only as representation evidence; message/follow/like it for that agency only when a first-party source separately proves the exact agency-owned profile URL.
...
When `campaignScope.enabled=true`, enforce its required countries and customer types before constructing both the potential pool and visible execution queue. For the current global-priority campaign, only large `key_account` buyers and `sales_agency`/brand-representation companies in the United States, Canada, United Kingdom, or South Africa qualify. Never fill a scoped shortfall with small unqualified retailers, out-of-scope companies, or another customer type; keep the gap visible and continue first-party enrichment instead.
...
- Facebook numeric page-id routes (`profile.php?id=<digits>`) are valid only when that exact URL is first-party cross-verified and marked `officialSocialProfileVerified=true`; keep generic/personal/non-numeric routes blocked. Normalize every pre-send target-validation error to `failed_open` with no-customer-interaction evidence and continue the safe queue without replay.
